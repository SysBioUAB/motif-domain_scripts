...
# Select host/pathogen and domain/motif.
host_pathogen = 'host'
...
motif_enrichment = subprocess.Popen('LC_NUMERIC=C awk \'{if ($2 + 0 > 1) {print $1}}\' outputs/host_domain_enrichment_01.txt',stdout=subprocess.PIPE,shell=True)
...
with open('outputs/'+host_pathogen+'_'+domain_or_motif+'domain_observed_freq_prueba.txt','rt') as f_obs:
	for line in f_obs:
...
with open('outputs/'+host_pathogen+'_'+domain_or_motif+'_random_freq_prueba.txt', 'rt') as f_rand:
	for line in f_rand:
...
# for each domain, if the domain is in the observed domains and it appears 10 or more times, take the observed frequency, then search this same domain in the random domains
# and compute the mean of all the 1000 frequencies. Finally, compute the fobs/frand for the selected domains.
for i in domains:
	try:
		if i in interactions_with_obs_freq:
			...
			if motif in motif_enrichment_only_enriched and domain in domain_enrichment_only_enriched:
				...
				if i in interactions_with_rand_freq and n >= 1:
					...
					with open('outputs/'+host_pathogen+'_'+domain_or_motif+'enrichment_nuevo_all.txt','at') as f:
